      ...
      pull.map(function (issue) {
        ...
        return '<section class="collapse">' +
          ...
          '<a href="' + u.encodeLink(issue.id) + '">' +
            u.formatMarkdownTitle(issue.title) +
...
I.serveRepoIssue = function (req, repo, issue, path, postId) {
  ...
  return self.repo.serveRepoTemplate(req, repo, 'issues', null, title, cat([
    pull.once(
      '<h3>' + u.link([issue.id], u.formatMarkdownTitle(issue.title)) + '</h3>' +
      '<code>' + issue.id + '</code>' +
  ...
  switch (c.type) {
    ...
    case 'post':
      if (c.root == issue.id) {
        ...
        return '<section class="collapse" id="' + id + '">' +
        ...
        return '<section class="collapse mention-preview" id="' + id + '">' +
