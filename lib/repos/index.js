    ...
    return cat([
      ...
      pull(
        ...
        pull.map(function (row) {
          return row ? '<tr>' + row.map(function (cell, i) {
            return '<td>' + cell + '</td>'
          }).join('') + '</tr>' : ''
        })
      ),
...
function renderRepoTree(req, repo, rev, path) {
  ...
  return cat([
    pull.once(pathLinks ? '<div class="fileLocation">' + req._t('Files') + pathLinks + '</div>' : ''),
    pull(
...
R.serveRepoObject = function (req, repo, sha1) {
  ...
  return u.readNext(function (cb) {
    repo.getObjectFromAny(sha1, function (err, obj) {
      if (err) return cb(null,
        self.web.serveBuffer(404, req._t('error.BlobNotFound')))
      cb(null, pull(obj.read, self.web.serveRaw(obj.length)))
    })
  })
}
      ...
      cb(null, cat([
        ...
        pull(
          ...
          pull.map(function (pack) {
            return 'P ' + pack.packId + '-' + pack.idxId + '.pack\n'
          })
        )
      ]))
    })
...
R.serveRepoPack = function (req, repo, name) {
  ...
  return u.readNext(function (cb) {
    if (m[3] === 'pack') {
      var packId = m[1]
      if (!ssbRef.isBlobId(packId)) return cb(null, pull.once([400]))
      repo.getPackfile(packId, function (err, read) {
        if (err) return cb(err)
        cb(null, pull(read,
    ...
    if (m[3] === 'idx') {
      var idxId = m[2]
      if (!ssbRef.isBlobId(idxId)) return cb(null, pull.once([400]))
      repo.getPackIndex(idxId, function (err, read) {
        if (err) return cb(err)
        cb(null, pull(read,
