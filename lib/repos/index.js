      ...
      cb(null, self.web.serveTemplate(req, title)(cat([
        h('div', {class: 'repo-title'}, [
          h('form', {class: 'right-bar', action: '', method: 'post'}, [
            ...
            h('strong', u.link(digsPath, votes.upvotes)),
            (isPublic
              ? once('')
            ...
            h('a', {href: u.encodeLink([repo.id, 'forks']), title: req._t('Forks')}, '+')
          ]),
          forms.name(req, !isPublic, repo.id, repoName, 'repo-name', null, req._t('repo.Rename'),
            h('h2', {class: 'bgslash'}, `${u.link([repo.feed], authorName)} / ${u.link([repo.id], repoName)}`)
          ),
        ]),
        (!repo.upstream
          ? once('')
