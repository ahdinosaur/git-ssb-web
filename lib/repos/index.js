...
R.serveRepoNotFound = function (req, id, err) {
  ...
  (pull.values([
    ...
    '<p>' + req._t('error.RepoIdNotFound', id) + '</p>',
...
R.serveRepoTemplate = function (req, repo, page, branch, titleTemplate, body) {
  ...
  var cloneUrls = '<div class="clone-urls">' +
    '<select class="custom-dropdown clone-url-protocol" ' +
     'onchange="with(this.nextSibling.firstChild) {' +
       'value = this.value; select() }">' +
      ...
      cb(null, self.web.serveTemplate(req, title)(cat([
        h('div', {class: 'repo-title'}, [
          ...
          forms.name(req, isLocal, repo.id, repoName, 'repo-name', null, req._t('repo.Rename'),
            h('h2', {class: 'bgslash'},
              (authorName ? u.link([repo.feed], authorName) + ' / ' : '') +
  ...
  return u.readNext(function (cb) {
    repo.getCommitParsed(rev, function (err, commit) {
      ...
      cb(null, self.serveRepoTemplate(req, repo, null, rev, title, cat([
        ...
        self.renderDiffStat(req, [repo, repo], [commit.parents[0], commit.id], commit.id, repoBranch),
        pull.once('</section>')
      ])))
    })
  })
...
R.renderDiffStat = function (req, repos, treeIds, commit, repoBranch) {
    ...
    pull(
      ...
      paramap(function (item, cb) {
        ...
        getRepoObjectString(repos[1], item.id[lastI], modeI, done())
          ...
          if (err) return cb(err)
          var repo = repos[1]
          cb(null, htmlLineDiff(req, repo, repoBranch, commit, item.filename, item.filename,
            strOld, strNew,
...
function htmlLineDiff(req, repo, repoBranch, commit, filename, anchor, oldStr, newStr, blobHref,
    showViewLink) {
  return '<pre><table class="code">' +
    ...
    (oldStr.length + newStr.length > 200000
      ? '<tr><td class="diff-info" colspan=3>' + req._t('diff.TooLarge') + '<br>' +
        req._t('diff.OldFileSize', {bytes: oldStr.length}) + '<br>' +
        req._t('diff.NewFileSize', {bytes: newStr.length}) + '</td></tr>'
      : tableDiff(req, repo, repoBranch, commit, oldStr, newStr, filename)) +
    '</table></pre>'
}

function tableDiff(req, repo, repoBranch, commit, oldStr, newStr, filename) {
  var query = req._u.query
    ...
    return [header].concat(hunk.lines.map(function (line) {
      ...
      return '<tr id="' + u.escape(id) + '" class="' + trClass + '">' +
        ...
        (commit && query.comment === id ?
          '<tr><td colspan=4>' +
            forms.lineComment(req, repo, repoBranch, commit, filename, lineNums[lineNums.length-1]) +
          '</td></tr>'
        : '')
    }))
  })
