...
P.serveRepoPullReq = function (req, repo, pr, path, postId) {
  ...
  var title = u.formatMarkdownTitle(pr.title) + ' · %{author}/%{repo}'
  return self.repo.serveRepoTemplate(req, repo, 'pulls', null, title, cat([
    pull.once('<div class="pull-request">' +
      '<h3>' + u.link([pr.id], u.formatMarkdownTitle(pr.title)) + '</h3>' +
      '<code>' + pr.id + '</code>'),
...
P.renderPullReqCommits = function (req, pr, baseRepo, headRepo, cb) {
  ...
  self.web.pullReqs.getRevs(pr.id, function (err, revs) {
    if (err) return cb(err)
    if (!revs.head) return cb(null, pull.once('<section>' +
      req._t('pullRequest.NoCommits') + '</section>'))
...
P.renderPullReqFiles = function (req, pr, baseRepo, headRepo, cb) {
  ...
  self.web.pullReqs.getRevs(pr.id, function (err, revs) {
    if (err) return cb(err)
    if (!revs.head) return cb(null, pull.once('<section>' +
      req._t('pullRequest.NoChanges') + '</section>'))
    GitRepo.getMergeBase(baseRepo, revs.base, headRepo, revs.head,
      function (err, mergeBase) {
        if (err) return cb(err)
        getObjectMsgId(headRepo, revs.head, function (err, objMsgId) {
          if (err) return cb(err)
          cb(null, cat([
            pull.once('<section>'),
            self.repo.renderDiffStat(req,
              [baseRepo, headRepo], [mergeBase, revs.head], revs.head, objMsgId),
            pull.once('</section>')
          ]))
        })
      }
...
P.renderPullReqActivity = function (req, pr, repo, headRepo, authorLink, postId) {
  ...
  return cat([
    ...
    !self.web.isPublic && pr.open && pull.once(`
      ...
      <pre>
git fetch ssb://${u.escape(pr.headRepo)} ${u.escape(pr.headBranch)}
git checkout -b ${u.escape(pr.headBranch)} FETCH_HEAD</pre>
      <p>${req._t('mergeInstructions.MergeAndPush')}</p>
      <pre>
git checkout ${u.escape(pr.baseBranch)}
git merge ${u.escape(pr.headBranch)}
git push ssb ${u.escape(pr.baseBranch)}</pre>
      </div></section>`),
...
P.serveRepoCompare = function (req, repo) {
  ...
  return self.repo.serveRepoTemplate(req, repo, 'pulls', null, title, cat([
    ...
    pull.once('<div class="bgslash">' +
      '<input type="radio" name="head" value="other" id="other-radio"> ' +
      '<label for="other-radio">other</label>: ' +
      '<input name="other_repo" placeholder="repo id"> / ' +
      '<input name="other_branch" placeholder="branch"></div>'),
    pull.once('</section>'),
    u.readOnce(function (cb) {
      cb(null,
...
P.serveRepoComparing = function (req, repo) {
  ...
  if (!ssbRef.isMsgId(headRepoId) && !/^#/.test(headRepoId))
    return self.web.serveError(req, new Error('bad repo id: ' + headRepoId), 400)
  ...
  return self.repo.serveRepoTemplate(req, repo, 'pulls', null, pageTitle, cat([
      ...
      pull.once('<section>' +
        ...
        : '<section><form method="get" action="">' +
          u.hiddenInputs({
            base: baseBranch,
            head: query.head,
            other_repo: query.other_repo,
            other_branch: query.other_branch,
          }) +
      ...
      u.readNext(function (cb) {
        GitRepo.getMergeBase(repo, baseBranch, headRepo, headBranch,
          function (err, concestor) {
            if (err) return cb(err)
            cb(null, cat([
              pull.once('<section id="files-tab">'),
...
P.renderCommitLog = function (req, baseRepo, baseBranch, headRepo, headBranch) {
  return cat([
    pull.once('<table class="compare-commits w-100">'),
    u.readNext(function (cb) {
